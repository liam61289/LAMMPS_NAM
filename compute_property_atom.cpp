  ...
  for (int iarg = 3; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      if (!atom->friction_flag) 		
        error->all(FLERR,"Compute property/atom for " 		
                   "atom property that isn't allocated"); 		
      pack_choice[i] = &ComputePropertyAtom::pack_ffricx; 		
    } else if (strcmp(arg[iarg],"ffricy") == 0) { 		
      if(!atom->friction_flag) 		
        error->all(FLERR,"Compute property/atom for " 		
                   "atom property that isn't allocated"); 		
      pack_choice[i] = &ComputePropertyAtom::pack_ffricy; 		
    } else if (strcmp(arg[iarg],"ffricz") == 0) { 		
      if (!atom->friction_flag) 		
        error->all(FLERR,"Compute property/atom for " 		
                   "atom property that isn't allocated"); 		
      pack_choice[i] = &ComputePropertyAtom::pack_ffricz; 		      	 		
    } else if (strcmp(arg[iarg],"ldamp") == 0) { 		
      if (!atom->friction_flag) 		
        error->all(FLERR,"Compute property/atom for " 		
                   "atom property that isn't allocated"); 		
      pack_choice[i] = &ComputePropertyAtom::pack_ldamp;
...
/* ---------------------------------------------------------------------- */		
void ComputePropertyAtom::pack_ffricx(int n)		
{		
  double **ffric = atom->ffric;		
  int *mask = atom->mask;		
  int nlocal = atom->nlocal;		
  for (int i = 0; i < nlocal; i++) {		
    if (mask[i] & groupbit) buf[n] = ffric[i][0];		
    else buf[n] = 0.0;		
    n += nvalues;		
  }		
}		
/* ---------------------------------------------------------------------- */		
void ComputePropertyAtom::pack_ffricy(int n)		
{		
  double **ffric = atom->ffric;		
  int *mask = atom->mask;		
  int nlocal = atom->nlocal;		
  for (int i = 0; i < nlocal; i++) {		
    if (mask[i] & groupbit) buf[n] = ffric[i][1];		
    else buf[n] = 0.0;		
    n += nvalues;		
  }		
}		
/* ---------------------------------------------------------------------- */		
void ComputePropertyAtom::pack_ffricz(int n)		
{		
  double **ffric = atom->ffric;		
  int *mask = atom->mask;		
  int nlocal = atom->nlocal;		
  for (int i = 0; i < nlocal; i++) {		
    if (mask[i] & groupbit) buf[n] = ffric[i][2];		
    else buf[n] = 0.0;		
    n += nvalues;		
  }		
}		
/* ---------------------------------------------------------------------- */		
void ComputePropertyAtom::pack_ldamp(int n)		
{		
  double *ldamp = atom->ldamp;		
  int *mask = atom->mask;		
  int nlocal = atom->nlocal;		
  for (int i = 0; i < nlocal; i++) {		
    if (mask[i] & groupbit) buf[n] = ldamp[i];		
    else buf[n] = 0.0;		
    n += nvalues;		
  }		
}		

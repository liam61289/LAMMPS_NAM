...
enum{ID,MOL,PROC,PROCP1,TYPE,ELEMENT,MASS,
     ...
     IX,IY,IZ,
     VX,VY,VZ,FX,FY,FZ,FFRICX,FFRICY,FFRICZ,LDAMP,
...
int DumpCustom::count()
{
  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      } else if (thresh_array[ithresh] == FFRICX) { 		
        if (!atom->friction_flag) 		
          error->all(FLERR, 		
                     "Threshold for an atom property that isn't allocated"); 		
 	    ptr = &atom->ffric[0][0]; 		
 	    nstride = 3; 		
      } else if (thresh_array[ithresh] == FFRICY) { 		
 	    if (!atom->friction_flag) 		
 	      error->all(FLERR, 		
 	                 "Threshold for an atom property that isn't allocated"); 		
 	    ptr = &atom->ffric[0][1]; 		
 	    nstride = 3; 		
      } else if (thresh_array[ithresh] == FFRICZ) { 		
        if (!atom->friction_flag) 		
 	    error->all(FLERR, 		
 	               "Threshold for an atom property that isn't allocated"); 		
 	    ptr = &atom->ffric[0][2]; 		
 	    nstride = 3; 		
      } else if (thresh_array[ithresh] == LDAMP) { 		
 	    if (!atom->friction_flag) 		
 	    error->all(FLERR, 		
 	               "Threshold for an atom property that isn't allocated"); 		
 	    ptr = atom->ldamp; 		
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    } else if (strcmp(arg[iarg],"ffricx") == 0) { 		
      if (!atom->friction_flag) 		
 	    error->all(FLERR,"Dumping an atom property that isn't allocated"); 		
      pack_choice[i] = &DumpCustom::pack_ffricx; 		
      vtype[i] = DOUBLE; 		
    } else if (strcmp(arg[iarg],"ffricy") == 0) { 		
      if (!atom->friction_flag) 		
        error->all(FLERR,"Dumping an atom property that isn't allocated"); 		
      pack_choice[i] = &DumpCustom::pack_ffricy; 		
      vtype[i] = DOUBLE; 		
    } else if (strcmp(arg[iarg],"ffricz") == 0) { 		
      if (!atom->friction_flag) 		
        error->all(FLERR,"Dumping an atom property that isn't allocated"); 		
      pack_choice[i] = &DumpCustom::pack_ffricz; 		
      vtype[i] = DOUBLE; 		
    } else if (strcmp(arg[iarg],"ldamp") == 0) { 		
      if (!atom->friction_flag) 		
        error->all(FLERR,"Dumping an atom property that isn't allocated"); 		
      pack_choice[i] = &DumpCustom::pack_ldamp; 		
      vtype[i] = DOUBLE;
...
int DumpCustom::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"ffricx") == 0) thresh_array[nthresh] = FFRICX; 		
    else if (strcmp(arg[1],"ffricy") == 0) thresh_array[nthresh] = FFRICY; 		
    else if (strcmp(arg[1],"ffricz") == 0) thresh_array[nthresh] = FFRICZ; 		
    else if (strcmp(arg[1],"ldamp") == 0) thresh_array[nthresh] = LDAMP; 
...
void DumpCustom::pack_ffricx(int n) 		
{ 		
  double **ffric = atom->ffric; 		
   		
  for (int i = 0; i < nchoose; i++) { 		
    buf[n] = ffric[clist[i]][0]; 		
    n += size_one; 		
  } 		
} 		
/* ---------------------------------------------------------------------- */ 		
void DumpCustom::pack_ffricy(int n) 		
{ 		
  double **ffric = atom->ffric; 		
   		
  for (int i = 0; i < nchoose; i++) { 		
    buf[n] = ffric[clist[i]][1]; 		
    n += size_one; 		
  } 		
} 		
/* ---------------------------------------------------------------------- */ 		
void DumpCustom::pack_ffricz(int n) 		
{ 		
  double **ffric = atom->ffric; 		
   		
  for (int i = 0; i < nchoose; i++) { 		
    buf[n] = ffric[clist[i]][2]; 		
    n += size_one; 		
  } 		
} 		
/* ---------------------------------------------------------------------- */ 		
void DumpCustom::pack_ldamp(int n) 		
{ 		
